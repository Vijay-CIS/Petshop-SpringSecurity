...
//	}
	...
	@RequestMapping(value = "/userPet", method = RequestMethod.GET)
	public String myPet(Model model, HttpSession session) {
		User user = (User) session.getAttribute("user");
		System.out.println("mypet Started(get)-user" + user);
		if (user == null) {
			return "redirect:/signin";
		}
		...
		model.addAttribute("plist", list);
		System.out.println("mypet Completed(get)");
		return "myPet";
	}
...
//	}
	...
	@RequestMapping(value = "/updatePetAvailableStatus/{petId}", method = RequestMethod.GET)
	public String buyProduct(@PathVariable("petId") Long petId, @RequestParam("status") boolean status,
			HttpSession session) {
		System.out.println("updatePetAvailableStatus started - get");
		Pet pet = petService.findByPetId(petId);
		pet.setAvailable(status);

		petService.updatePet(pet);
		System.out.println("updatePetAvailableStatus completed -get");
		return "redirect:/userPet";
	}
