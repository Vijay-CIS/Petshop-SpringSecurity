		...
		<div align="center">
			...
			<div>
				<c:if test="${sessionScope.user !=null }">
					<a href="newPet">Create New Pet</a>
				</c:if>
			</div>
				...
				<c:forEach var="pet" items="${plist}">
					<tbody>
						<tr>
							...
							<!-- <td>${pet.available}</td> -->
							<td><c:if
									test="${pet.available && (sessionScope.user==null || sessionScope.user.userId!=pet.user.userId)}">
									<a class="btn btn-success" href="/buyproduct/${pet.petId}">Buy</a>
								</c:if> <c:if test="${not pet.available}">
